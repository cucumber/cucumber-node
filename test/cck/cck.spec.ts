...
const UNSUPPORTED = [
  'pending', // node:test treats pending as success
...
describe('Cucumber Compatibility Kit',  () => {
  ...
  for (const ndjsonPath of ndjsonPaths) {
    const [, name, extension] = /^.+[/\\](.+)(\.feature(?:\.md)?)\.ndjson$/.exec(ndjsonPath) as RegExpExecArray
    ...
    it(name, async function() {
      ...
      await harness.copyFile(path.join(
        CCK_PATH,
        'features',
        name,
        name + extension,
      ...
      expect(actualEnvelopes.flatMap(envelope => Object.keys(envelope)))
        .to.deep.eq(expectedEnvelopes.flatMap(envelope => Object.keys(envelope)))

      // now get into the contents, excluding non-deterministic fields
      expect(actualEnvelopes)
        .excludingEvery(IGNORABLE_KEYS)
        .to.deep.eq(expectedEnvelopes)
    })
  }
})
